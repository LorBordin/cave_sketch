...
def settings_panel_component():
    ...
    with col2:
        marker_zoom = st.number_input(
            "🔍 Marker zoom [-1, 1]", min_value=-1., max_value=1., value=0., step=.1
        )
        text_zoom = st.number_input(
            "🔠 Text zoom [-1, 1]", min_value=-1., max_value=1., value=0., step=.1
        )
        line_width_zoom = st.number_input(
            "📏 Line width zoom [-1, 1]", min_value=-1., max_value=1., value=0., step=.1
        )
