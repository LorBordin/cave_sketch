...
def parse_dxf(input_path: Path, output_path: Optional[Path] = None) -> CaveSurvey:
    ...
    for idx, item in stations.items():
        ...
        links = [link.strip() for link in links_str.split("-") if link.strip()] if links_str else []
        x, y = item["point"]
        survey.points.append(
            SurveyPoint(
                id=str(idx), x=x - offset_x, y=y - offset_y, point_type="station", links=links
            )
        )
    ...
    for i, polyline in enumerate(all_polylines):
        ...
        for j, (x, y) in enumerate(pts):
            ...
            p = SurveyPoint(
                id=node_id, x=x - offset_x, y=y - offset_y, point_type=p_type, links=links
            )
