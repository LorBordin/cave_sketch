...
def test_merge_surveys_id_remapping():
    ...
    # Wait, the spec says child's matching station coincides with parent's.
    # The coincident node from child is REMOVED from the child dataframe before merging (as in merge_surveys.py)
    # OR it is mapped to the parent's ID.
    ...
    assert "3" not in node_ids # Child 1 was the matching node, should be merged with Parent 2
...
def test_merge_surveys_mixed_ids():
    ...
    # Parent: 1, 1P1
    # Child: 1 (mapped to 1), 1P1 (mapped to 2P1)
    # Offset for numeric: max(1) + 1 - min(1) = 1.
    # Offset for mixed: max(1) + 1 - min(1) = 1.
